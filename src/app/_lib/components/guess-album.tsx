...
import {
  ...
  Puzzle,
  recordResult,
} from '@/app/_lib/services/guess-album';
...
function pixelSrc(cover: string, w: number): string {
  return `/_next/image?url=${encodeURIComponent(cover)}&w=${w}&q=75`;
}

function shareText(results: boolean[]): string {
  const squares = results.map((r) => (r ? '🟩' : '⬛')).join('');
  const correct = results.filter(Boolean).length;
  return `🎵 jawr · guess the album · ${getTodayLabel()}\n\n${squares} ${correct}/${results.length}\n\njawr.org`;
}

type Translate = (key: string, opts?: Record<string, unknown>) => string;

function resultMessage(t: Translate, results: boolean[]): string {
  const correct = results.filter(Boolean).length;
  const total = results.length;
  if (correct === 0) return t('guess_album_result_none');
  if (correct === total) return t('guess_album_result_perfect', { total });
  return t('guess_album_result', { correct, total });
}
...
export function GuessAlbum() {
  ...
  async function handleShare() {
    const didCopy = await shareOrCopy(shareText(results));
    if (!didCopy) return;
    setCopied(true);
    timers.current.push(
    ...
    <section className="flex flex-col gap-4 items-center">
      ...
      {open && (
        <div className="flex flex-col gap-4 items-center w-full max-w-sm">
            ...
            <div className="flex flex-col items-center gap-4 text-center">
              {results.length > 0 && (
                <>
                  ...
                  >
                    {copied ? t('guess_album_copied') : t('guess_album_share')}
                  </button>
                </>
              )}
              <p className="text-[10px] text-gray-400 dark:tk-muted">
                {t('guess_album_tomorrow')}
              </p>
            </div>
          )}
          ...
          {puzzle && (
            <>
              <div className="relative w-80 aspect-square bg-gray-100 dark:bg-white/5 overflow-hidden">
                {!puzzle.cover ? (
                  ...
                  >
                    {PIXEL_STEPS.map((w, i) => (
                      ...
                      <img
                        key={w}
                        src={pixelSrc(puzzle.cover, w)}
                        alt={i === pixelStep ? puzzle.title : ''}
              ...
              <div className="grid grid-cols-2 gap-2 w-full">
                {puzzle.options.map((name) => {
                  const isCorrect = name === puzzle.artist;
              ...
              {done && (
                <div className="flex flex-col items-center gap-0.5 text-center">
                  ...
                  >
                    {solved ? t('guess_album_correct') : t('guess_album_wrong')}
                  </span>
                  <button
