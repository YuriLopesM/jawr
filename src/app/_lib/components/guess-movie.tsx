...
import {
  ...
  Puzzle,
  recordResult,
} from '@/app/_lib/services/guess-movie';
...
function pixelSrc(poster: string, w: number): string {
  return `/_next/image?url=${encodeURIComponent(poster)}&w=${w}&q=75`;
}

function shareText(results: boolean[]): string {
  const squares = results.map((r) => (r ? '🟩' : '⬛')).join('');
  const correct = results.filter(Boolean).length;
  return `🎬 jawr · guess the movie · ${getTodayLabel()}\n\n${squares} ${correct}/${results.length}\n\njawr.org`;
}
...
function resultMessage(t: Translate, results: boolean[]): string {
  ...
  if (correct === 0) return t('guess_movie_result_none');
  if (correct === total) return t('guess_movie_result_perfect', { total });
  return t('guess_movie_result', { correct, total });
}
    ...
    <section className="flex flex-col gap-4 items-center">
      ...
      {open && (
        <div className="flex flex-col gap-4 items-center w-full max-w-sm">
            ...
            <div className="flex flex-col items-center gap-4 text-center">
              {results.length > 0 && (
                <>
                  ...
                  >
                    {copied ? t('guess_movie_copied') : t('guess_movie_share')}
                  </button>
                </>
              )}
              <p className="text-[10px] text-gray-400 dark:tk-muted">
                {t('guess_movie_tomorrow')}
              </p>
            </div>
          )}
          ...
          {puzzle && (
            <>
              <div className="relative w-56 aspect-[2/3] bg-gray-100 dark:bg-white/5 overflow-hidden">
                {!puzzle.poster ? (
              ...
              <div className="grid grid-cols-2 gap-2 w-full">
                {puzzle.options.map((name) => {
                  const isCorrect = name === puzzle.title;
              ...
              {done && (
                <div className="flex flex-col items-center gap-0.5 text-center">
                  ...
                  >
                    {solved ? t('guess_movie_correct') : t('guess_movie_wrong')}
                  </span>
                  <button
